  ...
  $.fn.canvas_image_detail = function (options, element) {
    ...
    class ImageDetail {
      ...
      image_on_load(img) {
        ...
        var hcanvas = $(
          `<canvas width="${whole_width}px" height="${whole_height}px" tabindex="0"/>`
        );
      ...
      show_image(imgArray, whole_width, whole_height) {
        ...
        this.gamma_value = $("<div>gamma_value here.</div>").appendTo(
          gamma_area
        );
        this.gamma_value.addClass("gamma-value");
        $("<div> &nbsp; Zoom: </div>").appendTo(gamma_area);
        var zoom_select = $(
          '<select>  <option value="1">1x</option> </select>'
        ).appendTo(gamma_area);
        for (var i = 2; i < 5; i++) {
        ...
        this.update_images();
        this.detail_image = detail.named_image({
          image_name: "example",
          x: 0,
          y: 0,
          w: whole_width,
        ...
        var whole_image = whole.named_image({
          image_name: "example",
          x: 0,
          y: 0,
          w: canvas_width,
        ...
        var init_position = {
          x: (whole_width - canvas_width) / 2,
          y: (whole_height + canvas_height) / 2,
        };
        this.set_position(init_position);

        // set up controls
        var slide_change = function (event) {
          that.gamma_slide(event);
        };
        slider.empty();
        slider.width(500);
        slider.slider({
        ...
        zoom_select.on("change", function (event) {
          that.zoom_change(event);
        });
      ...
      update_images() {
        ...
        this.whole_canvas.name_image_data(
          "example",
          this.corrected_array,
          this.whole_width,
          this.whole_height
        );
        this.detail_canvas.name_image_data(
          "example",
          this.corrected_array,
          this.whole_width,
          this.whole_height
