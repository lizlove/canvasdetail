...
(function ($) {
  ...
  $.fn.dual_canvas_helper = function (options) {
    ...
    target.fit = function (stats, margin) {
      ...
      if (!margin) {
        margin =
          0.01 * Math.max(stats.max_x - stats.min_x, stats.max_y - stats.min_y); // xxxx?
      }
      ...
      for (var event_type in target.event_info.event_types) {
        target.visible_canvas.canvas.on(
          event_type,
    ...
    $.fn.dual_canvas_helper.draw_op_names = (
      "circle line text rect polygon " + "named_image assembly"
    ).split(" ");
    ...
    target.store_object_info = function (info, draw_on_canvas, in_place) {
      ...
      if (name) {
        ...
        object_info.transition =
          object_info.transition ||
          function (to_values, seconds_duration) {
    ...
    target.name_image_data = function (
      image_name,
      data_array,
      width,
      height,
        ...
        for (var i = 0; i < wh; i++) {
          ...
          for (var j = 0; j < 4; j++) {
            ...
            data[index] = Math.floor(
              lambda1 * low_color[j] + lambda * high_color[j]
            );
          }
        }
      } else {
    ...
    target.abbreviated_on_canvas_event = function (
      event_type,
      callback,
      for_name_or_info,
      delayed
    ...
    target.on_canvas_event = function (event_type, callback, for_name_or_info) {
      if (for_name_or_info) {
        ...
        if (object_info) {
          ...
          console.warn(
            "in on_canvas_event no object found with name: " + for_name
          );
        }
      } else {
    ...
    target.off_canvas_event = function (event_type, for_name_or_info) {
      if (for_name_or_info) {
        ...
        if (object_info && n2t2h[for_name] && n2t2h[for_name][event_type]) {
          ...
          console.warn(
            "in off_canvas_event no bound object found with name: " + for_name
          );
        }
      } else {
    ...
    target.object_name_at_position = function (event, pixel_x, pixel_y) {
      ...
      var color_index = target.color_index_at(
        target.invisible_canvas,
        pixel_x,
        pixel_y
      );
    ...
    target.deferred_event_handler = function (e) {
      ...
      var process_event = function (e, no_default) {
        ...
        if (!no_default) {
          default_handler =
            target.event_info.default_event_handlers[event_type];
        }
        // object specific handling for registered objects
        if (
          e.canvas_name &&
          !target.disable_element_events &&
          target.name_to_object_info[e.canvas_name]
        ) {
      ...
      if (
        last_event &&
        last_event.canvas_name &&
        (!e.canvas_name || e.canvas_name != last_event.canvas_name)
      ) {
      ...
      if (
        ...
        (!last_event ||
          !last_event.canvas_name ||
          last_event.canvas_name != e.canvas_name)
      ) {
    ...
    target.vector_frame = function (x_vector, y_vector, xy_offset, name) {
    ...
    target.callback_with_pixel_color = function (
      pixel_x,
      pixel_y,
      callback,
      delay
    ...
    target.shaded_objects = function (shading_name_or_info) {
      ...
      for (var i = 0; i < shaded_pixels.length; i += 4) {
        ...
        if (shading_color == pseudocolor) {
          ...
          var shaded_color_index =
            target.color_array_to_index(shaded_color_array);
          var shaded_object_name =
            target.color_index_to_name[shaded_color_index];
          if (shaded_object_name) {
            var shaded_object_info =
              target.name_to_object_info[shaded_object_name];
            if (shaded_object_info) {
              ...
              if (!shaded_object_evidence[shaded_object_name]) {
                shaded_object_evidence[shaded_object_name] = {
                  x: pixel_x,
                  y: pixel_y,
                  info: shaded_object_info,
                };
              }
            }
          }
      ...
      for (var name in shaded_object_evidence) {
        ...
        if (
          target.object_name_at_position(null, evidence.x, evidence.y) == name
        ) {
      ...
      var pixel_in_shadow = function (px, py) {
        ...
        if (
          row >= 0 &&
          row < shaded_info.height &&
          px >= 0 &&
          px < shaded_info.width
        ) {
    ...
    target.color_string_to_array = function (color_string) {
      ...
      if (color_string.startsWith("rgba")) {
        ...
        try {
          ...
          if (
            parenthesized.substring(0, 1) == "(" &&
            parenthesized.substring(last, last + 1) == ")"
          ) {
      ...
      var p = target.pixel_offset(
        0.5 * (bbox.min_x + bbox.max_x),
        0.5 * (bbox.max_y + bbox.min_y)
      );
    ...
    target.transition = function (
      ...
      var transition = {
        ...
        interpolator: target.linear_interpolator(
          object_name,
          from_values,
          to_values
        ),
    ...
    target.linear_interpolator = function (
      object_name,
      from_mapping,
      to_mapping
    ) {
      ...
      for (var attr in to_mapping) {
        ...
        if (from_value != to_value) {
          ...
          if (to_value_type == "number") {
            ...
            map_interpolators[attr] = target.linear_numeric_interpolator(
              from_value,
              to_value
            );
            trivial = false;
            ...
            if (attr == "color" || attr == "background") {
              map_interpolators[attr] = target.color_interpolator(
                from_value,
                to_value
              );
              trivial = false;
            } else if (attr == "points") {
              map_interpolators[attr] = target.polygon_points_interpolator(
                from_value,
                to_value
              );
              trivial = false;
              ...
              map_interpolators[attr] = target.switch_value_interpolator(
                from_value,
                to_value
              );
              trivial = false;
    ...
    var draw_star = function (assembler, options) {
      ...
      for (var i = 0; i < settings.points; i++) {
        coords.push([
          x + Math.cos(theta) * outer_radius,
          y + Math.sin(theta) * outer_radius,
        ]);
        theta += dtheta;
        coords.push([
    ...
    var draw_arrow = function (assembler, options) {
      ...
      if (len > epsilon) {
        ...
        var start = target.vadd(target.vscale(-len + head_offset, to_p1), p1);
    ...
    var draw_double_arrow = function (assembler, options) {
      ...
      if (len > epsilon) {
        ...
        if (settings.backward) {
          ...
          b.head_angle = numeric_default(
            settings.back_angle,
            settings.head_angle
          );
          b.head_offset = numeric_default(
            settings.back_offset,
            settings.head_offset
          );
          b.assembly = "arrow";
  ...
  $.fn.dual_canvas_helper.add_geometry_logic = function (target) {
    ...
    target.color_chooser = function (config, element) {
      ...
      var preview = misc_frame.circle({
        name: true,
        x: swatch_offset,
        y: side - swatch_offset,
        r: swatch_offset,
        color: "white",
      });
      ...
      var final_color = misc_frame.rect({
        name: true,
        hide: true,
        x: 10,
        y: 10,
      ...
      var color_choice = misc_frame.text({
        name: true,
        x: 10 + (left - 20) * 0.5,
        y: bottom * 0.3 + 10,
    ...
    target.bottom_axis = function (
      ...
      var min_value = numeric_default(
        params.min_value,
        stats["min_" + coordinate]
      );
      var max_value = numeric_default(
        params.max_value,
        stats["max_" + coordinate]
      );
      if (!params.ticks) {
        // infer ticks from limits
        params.ticks = target.axis_ticklist(
          min_value,
          max_value,
          params.max_tick_count,
          params.anchor
        );
        //params.ticks = ticklist.map(x => {offset: x});
      }
      // convert numeric ticks to mappings
      params.ticks = params.ticks.map(function (x) {
        if (typeof x == "number") {
          return { offset: x };
        } else {
          return x;
    ...
    target.axis = function (config) {
      ...
      var tick_direction_length = target.vlength(
        target.vadd(
          target.converted_location(
            params.tick_direction.x,
            params.tick_direction.y
          ),
          target.vscale(-1, target.converted_location(0, 0))
        )
      );
      var tick_model_conversion = 1.0 / tick_direction_length;
      var tick_shift = target.vscale(
        params.tick_length * tick_model_conversion,
        params.tick_direction
      );
      var label_shift = target.vscale(
        params.label_offset * tick_model_conversion,
        params.tick_direction
      );
      ...
      for (var i = 0; i < ticks.length; i++) {
        ...
        tick.start = target.vadd(tick.origin, target.vscale(-1, tick_shift));
        tick.end = target.vadd(tick.origin, tick_shift);
        line.x1 = tick.start.x;
        line.y1 = tick.start.y;
        line.x2 = tick.end.x;
        line.y2 = tick.end.y;
        // draw the line mark
        target.line(line);
        ...
        label.valign = "center";
        label.offset = target.vadd(tick.origin, label_shift);
      ...
      if (min_tick && params.connecting_line_config) {
        var connecting_line = $.extend(
          {},
          params.tick_line_config,
          params.connecting_line_config
        );
  ...
  $.fn.dual_canvas_helper.reference_frame = function (
    ...
    frame.install_assembly = function (name, draw_on_target) {
      ...
      frame.parent_canvas.install_assembly(name, draw_on_target, frame);
    };
    ...
    frame.coordinate_conversion = function (
      position,
      attribute_name,
      coordinate_names
    ) {
      var nc = frame.parent_canvas.no_change_conversion(
        position,
        attribute_name,
        coordinate_names
      );
    ...
    frame.model_location = function (mx, my) {
      ...
      // untranslate
      var untranslated = {
        x: mx - frame.xy_offset.x,
        y: my - frame.xy_offset.y,
      };
      // then "unscale"
    ...
    var override_positions = function (shape_name) {
      ...
      frame[shape_name] = function (opt, wait) {
        ...
        var s = opt;
        s.coordinate_conversion =
          s.coordinate_conversion || frame.coordinate_conversion;
  ...
  $.fn.dual_canvas_helper.example = function (element, x, y, w, h) {
    ...
    var info = $("<div>click the circle to pick it up..</div>").appendTo(
      element
    );
  ...
  $.fn.dual_canvas_helper.lasso_example = function (element) {
    ...
    $("<div>Please lasso some circles once to turn them pink.</div>").appendTo(
      element
    );
  };
