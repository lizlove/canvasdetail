module.exports = async function canvas_2d_widget_helper(options) {
  let target = this; //Can we refactor this out?
  let defaults = {
  ...
  target.reset_canvas = function () {
    ...
    target.canvas = $(
      `<canvas width="${w}px" height="${h}px" style="${st}" tabindex="0"/>`
    );
  ...
  var no_change_conversion = function (
    position,
    attribute_name,
    coordinate_names
  ) {
    ...
    if (Array.isArray(position)) {
      ...
      result = {
        x: position[coordinate_names[0]],
        y: position[coordinate_names[1]],
      };
    }
    return result;
  };
  ...
  target.rectangle_stats = function (
    s,
    w,
    h,
    degrees,
  ...
  target.named_image = function (opt) {
    ...
    if (isnum(s.sx) && isnum(s.sy) && isnum(s.sWidth) && isnum(s.sHeight)) {
      try {
        ...
        console.warn(
          "failed to draw image window " + [s.image_name, err.message]
        );
      }
    } else {
      try {
        ...
        console.warn(
          "failed to draw whole image " + [s.image_name, err.message]
        );
      }
    }
    context.restore(); // matches translate_and_rotate
